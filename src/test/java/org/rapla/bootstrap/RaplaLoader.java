...
public class RaplaLoader
{
    ...
    private static ClassLoader getJarClassloader(String baseDir, String dirList) throws IOException, MalformedURLException, URISyntaxException
    {
        ...
        String[] split = classpathEntries != null ? classpathEntries.split(";") : new String[0];
        final ArrayList<URL> cpUrls = new ArrayList<URL>(
                Arrays.asList(urls));
        boolean googleDevOnClassPath = false;
        for (String cpEntry : split)
        {
            if (!cpEntry.contains("gwt-dev"))
            {
                cpUrls.add(new File(cpEntry).toURI().toURL());

            }
            else
            {
                googleDevOnClassPath = true;
            }
        }
        URL[] classLoaderUrls = googleDevOnClassPath ? cpUrls.toArray(new URL[cpUrls.size()]) : urls;
        ClassLoader parent = googleDevOnClassPath ? null : RaplaLoader.class.getClassLoader();
        final ClassLoader classLoader = new URLClassLoader(classLoaderUrls, parent);
        return classLoader;
    }
